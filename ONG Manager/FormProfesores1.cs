...
namespace ONG_Manager
{
	...
	public partial class FormProfesores1 : Form
	{
		...
		string sql;
		public FormProfesores1(bool editar, string ideditar)
		{
			//
		...
		void cargarprofeeditar(string idprofeeditar)
		{
			SQLiteConnection conn = new SQLiteConnection(strcon);
  			conn.Open();
			sql = "select * from PROFESORES where id ='"+idprofeeditar+"';";
			SQLiteCommand cmd = new SQLiteCommand(sql, conn);
			SQLiteDataReader r = cmd.ExecuteReader();
			while (r.Read()) {
				tb0.Text = r[0].ToString();
		...
		void Button3Click(object sender, EventArgs e)
		{
			SQLiteConnection conn = new SQLiteConnection(strcon);
  			conn.Open();
			sql = "UPDATE PROFESORES SET NOMBRE = '"+tb1.Text+"', APELLIDO1 = '"+tb2.Text+"' , APELLIDO2 = '"+tb3.Text+"', NIF = '"+tb4.Text+"',PAIS = '"+tb5.Text+"',EDAD = '"+tb6.Text+"' ,SEXO = '"+cb1.Text+"' ,TELEFONO1 = '"+tb7.Text+"',TELEFONO2 = '"+tb8.Text+"',EMAIL = '"+tb9.Text+"' ,DIRECCION1 = '"+tb10.Text+"',DIRECCION2 = '"+tb11.Text+"',POBLACION = '"+tb12.Text+"', PROVINCIA = '"+tb13.Text+"', OBSERVACIONES = '"+tb14.Text+"' WHERE ID = '"+tb0.Text+"';";
			SQLiteCommand cmd = new SQLiteCommand(sql, conn);
			cmd.ExecuteNonQuery();
			conn.Close();
